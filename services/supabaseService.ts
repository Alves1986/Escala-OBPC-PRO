...
export const fetchUserAllowedMinistries = async (userId: string, orgId: string): Promise<string[]> => {
    ...
    try {
        const { data: memberships, error } = await supabase.from('organization_memberships').select('ministry_id').eq('profile_id', userId).eq('organization_id', orgId);
        if (!error && memberships && memberships.length > 0) return memberships.map((m: any) => m.ministry_id);
...
export const fetchOrganizationMinistries = async (organizationId: string): Promise<MinistryDef[]> => {
    ...
    const { data, error } = await supabase.from('organization_ministries').select('id, code, label').eq('organization_id', orgId).order('label');
    if (error) return [];
    ...
    return data.map((m: any) => ({ id: m.id, code: m.code, label: m.label, enabledTabs: DEFAULT_TABS, organizationId: orgId }));
};
...
export const fetchMinistryMembers = async (ministryId: string): Promise<{ memberMap: MemberMap, publicList: TeamMemberProfile[] }> => {
    ...
    if (orgId && orgId !== DEFAULT_ORG_ID) query = query.eq('organization_id', orgId);
    ...
    const publicList: TeamMemberProfile[] = filteredData.map((p: any) => ({
        id: p.id, name: p.name, email: p.email, whatsapp: p.whatsapp, avatar_url: p.avatar_url, roles: safeParseArray(p.functions), birthDate: p.birth_date, isAdmin: p.is_admin, organizationId: p.organization_id
    }));
...
// --- FIX: MEMBER AVAILABILITY SAVING WITH MANUAL EXISTENCE CHECK ---
