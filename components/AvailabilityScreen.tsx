...
export const AvailabilityScreen: React.FC<Props> = ({ 
  ...
  const getWindowState = () => {
      ...
      // Checa se o ano é menor que 2000 (abrange 1970 e variantes de timezone)
      if (dbStart && new Date(dbStart).getFullYear() < 2000) return 'CLOSED';
      
      const start = dbStart ? new Date(dbStart) : null;
      const end = availabilityWindow?.end ? new Date(availabilityWindow.end) : null;
      
      if (!start || !end) return 'OPEN'; // Default open
      const now = new Date();
  ...
  const handleDayClick = (day: number) => {
      if (!isEditable) {
          addToast("Janela fechada pela liderança.", "error");
          return;
      }
      ...
      if (isSunday) {
          if (!existingEntry) {
              ...
              newDates.push(`${dateStr}_M`);
          } else if (existingEntry.endsWith('_M')) {
              newDates.push(`${dateStr}_N`);
          } 
      } else {
          if (!existingEntry) {
              newDates.push(dateStr);
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl mx-auto pb-24">
      ...
      {/* Header Controls */}
      <div className={`flex flex-col md:flex-row justify-between items-center gap-4 p-5 rounded-2xl border shadow-sm transition-colors duration-500 ${isEditable ? 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700' : 'bg-zinc-100 dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700'}`}>
          <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl transition-colors ${isEditable ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-zinc-200 text-zinc-500 dark:bg-zinc-800'}`}>
                  {isEditable ? <CalendarCheck size={24} /> : <Lock size={24}/>}
              </div>
              <div>
                  <h2 className="text-lg font-bold text-zinc-800 dark:text-white leading-tight">
                      {isEditable ? 'Disponibilidade' : 'Agenda Bloqueada'}
                  </h2>
                  <p className="text-sm text-zinc-500">
                      {isEditable ? 'Toque nos dias em que você PODE servir.' : 'Somente leitura neste momento.'}
                  </p>
              </div>
          </div>
      ...
      {selectedMember ? (
          <div className="relative">
              ...
              {/* Glassmorphism Lock Overlay - Visual Bloqueio */}
              {!isEditable && (
                  <div className="absolute inset-0 z-50 flex items-center justify-center rounded-3xl backdrop-blur-[2px] bg-white/40 dark:bg-black/40 transition-all duration-500">
                      <div className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-white p-8 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-700 flex flex-col items-center gap-4 max-w-sm text-center">
                          <div className="p-4 bg-red-100 dark:bg-red-900/30 rounded-full text-red-600 dark:text-red-400">
                              <Lock size={32} />
                          </div>
                          <div>
                              <h3 className="font-bold text-xl mb-1">Janela Fechada</h3>
                              <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
                                  O período para envio de disponibilidade foi encerrado pela liderança. Entre em contato para exceções.
                              </p>
                          </div>
                      </div>
                  </div>
              ...
              <div className={`transition-all duration-500 ${!isEditable ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
                  {/* Legend */}
