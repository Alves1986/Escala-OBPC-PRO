...
export const EventsScreen: React.FC<Props> = ({ customEvents, setCustomEvents, currentMonth, onMonthChange }) => {
  ...
  const handleAdd = () => {
    if (newDate && newTitle) {
      setCustomEvents([...customEvents, { id: Date.now().toString(), date: newDate, time: newTime, title: newTitle }]);
      setNewTitle("");
      // Opcional: manter a data ou limpar
    }
  };
      ...
      <div className="space-y-2">
        ...
        {customEvents.map(evt => (
          <div key={evt.id} className="flex justify-between items-center bg-white dark:bg-zinc-800 p-4 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm animate-fade-in">
            ...
            <button 
                onClick={() => setCustomEvents(customEvents.filter(e => e.id !== evt.id))} 
                className="text-red-500 bg-red-50 dark:bg-red-900/20 p-2 rounded-lg hover:bg-red-100 transition-colors"
            >
                <Trash2 size={18}/>
            </button>
          </div>
